...
**Hardware:** RTX Pro 6000 (96 GB VRAM) — estimated <1 hour.

Data auto-downloaded from GitHub. Fully self-contained.
""")
    return Path, json, mo, nn, random, torch, urllib


@app.cell(hide_code=True)
def _(Path, json, mo, nn, random, torch, urllib):
    # Step 1: Download training data from GitHub
    data_path = Path("/marimo/trm_content_review_25k.jsonl")
    if not data_path.exists():
        mo.md("⏳ Downloading 25K training examples from GitHub...")
        url = "https://raw.githubusercontent.com/Seamus05/bitmajor-trilogy/master/data/trm_content_review_25k.jsonl"
        urllib.request.urlretrieve(url, str(data_path))
        mo.md(f"✅ Downloaded: {data_path.stat().st_size / 1024 / 1024:.1f} MB")
    else:
        mo.md(f"✅ Data cached: {data_path.stat().st_size / 1024 / 1024:.1f} MB")

    examples = [json.loads(l) for l in open(data_path)]
    mo.md(f"✅ Loaded {len(examples)} examples")

    # Step 2: Byte-level tokenization (no tokenizer dependency)
    ...
    mo.md(f"✅ Tokenized: {len(train_inputs)} train, {len(eval_inputs)} eval")

    # Step 3: TRM Architecture (7M params, 2 layers, no attention)
    ...
    mo.md("⏳ Training (10 epochs, ~30-60 min on RTX Pro 6000)...")
    ...
    for epoch in range(n_epochs):
        ...
        if eval_acc > best_eval_acc:
            ...
            torch.save(model.state_dict(), "/marimo/trm_best.pt")
    ...
    model.load_state_dict(torch.load("/marimo/trm_best.pt"))
